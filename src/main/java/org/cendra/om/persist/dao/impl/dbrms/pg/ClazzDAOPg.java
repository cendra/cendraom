...
import org.cendra.om.model.clazz.datatype.DataTypeArray;
import org.cendra.om.model.clazz.datatype.DataTypeClazz;
import org.cendra.om.model.clazz.datatype.DataTypeClazzObject;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleBigDecimal;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleBoolean;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleDate;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleDouble;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleFloat;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleInteger;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleLong;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleShort;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleString;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleTime;
import org.cendra.om.model.clazz.datatype.DataTypeSimpleTimestamp;
...
public class ClazzDAOPg extends GenericDAO {
	...
	private void insertTableClazzAttribute(ConnectionWrapper connectionWrapper,
		...
		insertTableClazzAttribute(connectionWrapper, true, clazz.getId(),
				clazz.getAttributes());

	}

	private void insertTableClazzAttribute(ConnectionWrapper connectionWrapper,
			ClazzAttribute attribute) throws Exception {

		if (attribute.getDataType() != null
				&& attribute.getDataType() instanceof DataTypeClazzObject) {

			insertTableClazzAttribute(connectionWrapper, false,
					attribute.getId(),
					((DataTypeClazzObject) attribute.getDataType())
							.getAttributes());
		}
	...
	private void insertTableClazzAttribute(ConnectionWrapper connectionWrapper,
			boolean isMemberOfClazz, String memberOfId,
			List<ClazzAttribute> attributes) throws Exception {

		for (ClazzAttribute attribute : attributes) {
			...
			Object memberOfAttribute = String.class;
			...
			if (isMemberOfClazz == true && memberOfId != null) {
				memberOfClazz = memberOfId;
			} else if (isMemberOfClazz == false && memberOfId != null) {
				memberOfAttribute = memberOfId;
			}
			...
			String[] nameAtts = { "id", "code", "orderAtt", "name", "comment",
					"labelName", "labelComment", "memberOfClazz",
					"memberOfAttribute", "isArray", "dataTypeBoolean",
			...
			Object[] args = { id, code, orderAtt, name, comment, labelName,
					labelComment, memberOfClazz, memberOfAttribute, isArray,
			...
			if (attribute.getDataType() != null
				...
				insertTableClazzAttribute(connectionWrapper, attribute);
			}
	...
	private void createTable(ConnectionWrapper connectionWrapper, String sql,
		...
		for (ClazzAttribute attributeItem : attributes) {
			...
			if (attributeItem.getDataType() != null) {
				...
				} else if (attributeItem.getDataType() instanceof DataTypeClazzObject) {

					createTableObject(connectionWrapper, schemaName,
							tableNameSource, attributeItem);
	...
	private void createTableObject(ConnectionWrapper connectionWrapper,
			String schemaName, String tableNameSource, ClazzAttribute attribute)
			throws SQLExceptionWrapper, SQLException {

		String sql = "\n";

		String tableName = attribute.getSimplePath().replace(".", "_");
		...
		sql += "CREATE TABLE " + schemaName + "." + tableName;
		sql += "\n(";
		sql += "\n\tid VARCHAR(36) NOT NULL PRIMARY KEY DEFAULT uuid_generate_v4() REFERENCES "
				+ schemaName + "." + tableNameSource + " (id)";

		List<ClazzAttribute> attributes = ((DataTypeClazzObject) attribute
				.getDataType()).getAttributes();

		createTable(connectionWrapper, sql, schemaName, tableName, attributes);
			...
			if (attributeItem.getDataType() != null) {
				...
				if (isArray == false) {
					...
					if (dataType.trim().length() > 0) {
						...
						if (attributeItem.getName().trim().length() > 63) {
							throw new IllegalStateException(
									"It is not allowed to create an attribute whose Name exceeds the length of 63 characters.\nFull Path: "
											+ attributeItem.getPath()
											+ "\nSimple Path: "
											+ attributeItem.getSimplePath());
						}
	...
	private void createTableArray(ConnectionWrapper connectionWrapper,
		...
		if (attribute.getDataType() instanceof DataTypeClazzObject) {
			List<ClazzAttribute> attributes = ((DataTypeClazzObject) attribute
					.getDataType()).getAttributes();

			createTable(connectionWrapper, sql, schemaName, tableName,
					attributes);
		} else {
		...
		sql = "CREATE UNIQUE INDEX u_" + schemaName + "_" + tableName
				+ "_id_" + tableNameSource + "_id ON " + schemaName + "."
				+ tableName + " (TRIM(" + tableName + ".id), TRIM("
				+ tableName + "." + tableNameSource + "_id));";
