...
import org.cendra.om.model.clazz.datatype.DataTypeForClazz;
...
public class Clazz {
	...
	public List<ClazzAttribute> getFullAttributes() {
		
		List<ClazzAttribute> attributesFull = new ArrayList<ClazzAttribute>();

		for (ClazzAttribute attributeItem : attributes) {

			if (attributeItem.getDataType() != null
					&& attributeItem.getDataType() instanceof DataTypeClazzObject) {
				
				DataTypeClazzObject dt = (DataTypeClazzObject) attributeItem
						.getDataType();

				dt.getFullAttributes(attributesFull);


			} else {
				attributesFull.add(attributeItem);
			}
		}
	...
	public boolean addAttributeObject(String name) {
		return addAttribute(name, new DataTypeClazzObject());
	}
